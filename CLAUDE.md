...
### Faz 5.5 — Sohbet gerçek zamanlı iyileştirmeler · **Tamamlandı [24.07.2026 14:31]**

- **Tamamlandı [24.07.2026 14:31]** — Üç canlı iyileştirme: (1) **Admin canlı izleme** — `ChatHub`'a konuşma-bazlı SignalR grubu (`KonusmaGrubu`, sıra bağımsız) + `IzlemeyeBasla` (yalnız Admin, hub'da rol kontrolü); mesaj ilgili gruba da yayınlanır, admin sayfa yenilemeden görür (reconnect'te tekrar katılır). (2) **Site içi bildirim** — `_Layout`'ta global SignalR bağlantısı (`window.tesHub`); gelen mesaj için Tabler toast (tıklayınca konuşmaya gider) + WebAudio ile üretilen yumuşak "ding" (harici ses dosyası yok, CDN kuralı korunur); açık olan konuşma ve kendi mesajları için toast bastırılır. (3) **"Yazıyor..." + "Okundu"** — hub'a `Yaziyor` (throttle'lı, izin kontrollü) ve `OkunduBildir` (`SohbetServisi.OkunduIsaretleAsync` + karşıya anlık "Okundu"); okundu işaretleme artık controller'dan hub'a taşındı. Tüm yük/DOM güncellemeleri `textContent` ile (XSS güvenli). Testler: 2 yeni (okundu işaretleme + karşı gelen kutusunu etkilememe). Toplam **73/73 geçti**. `dotnet build` + `dotnet test` temiz; hub metotları anonim erişimde korumalı.

> **MVP faz planı tamamlandı (Faz 0-5 + Faz 5.5).** Sonraki adımlar (CLAUDE.md Bölüm 12): mobil iyileştirme, basit yerel AI, KVKK sertleştirme.

### Faz 6 — Responsive Navbar + scarlet-snow Tema (Aç/Koyu) · **Tamamlandı [24.07.2026 15:10]**

- **Tamamlandı [24.07.2026 15:10]** — İki görsel iyileştirme (yalnız UI; yetki/servis mantığına dokunulmadı). (1) **Navbar sıkışması giderildi:** rol-duyarlı menü açılır menülerle gruplandı — üst-seviye link sayısı admin'de ~11 → 6 (Yoklama ▾ = Yoklama Kayıtları/RFID Simülatör; Stajyer Takibi ▾ = Stajyerler/Verdiğim Ödevler; İşlerim ▾ = Projem/Ödevlerim; Yönetim ▾ = Departmanlar/Kullanıcı Yönetimi/Misafir Yönetimi/Moderasyon). Görünürlük yine `adminMi/amirMi/stajyerMi` bayraklarıyla; yetki sunucuda. (2) **scarlet-snow teması** (shadcn/ui kaynaklı) yalnız RENK PALETİ olarak taşındı — yeni CSS framework yok: Tabler 1.4.0 zaten `data-bs-theme="dark"` ile tam koyu tema sunuyor, `site.css`'te `--tblr-*` değişkenleri hem açık `:root` hem koyu `:root[data-bs-theme=dark]` için override edildi (Primary `#fb2c36`; açık bg `#fafafa`/card `#fdfdfd`, koyu bg `#0a0a0a`/card `#18181b`; danger açık `#e7000b`/koyu `#ff6467`). Değişken adları `tabler.min.css`'ten grep ile doğrulandı (`--tblr-card-bg`, `--tblr-bg-surface`, `--tblr-secondary-bg`, `--tblr-danger(-rgb)`, `--tblr-link-color`); `.btn-primary` hover/active dahil tümü `--tblr-primary`/`--tblr-primary-fg`'den beslendiği için override her yeri kapsıyor. (3) **Aç/Koyu geçiş:** navbar'a güneş/ay düğmesi (inline SVG — ikon fontu vendor'lı değil); tercih `localStorage`'da (`tes-tema`), ilk açılış sistem tercihine (`prefers-color-scheme`) uyar; FOUC engeli için tema, gövde render'ından ÖNCE `<head>` inline script'iyle `<html>`'e yazılır (iki layout'ta da). Mantık `site.js`'te; her iki layout `site.js`'i bağlar. (4) **Responsive:** liste tabloları zaten `.table-responsive` içindeydi, dashboard kartları `col-sm-6 col-lg-4` (telefonda tek sütun) — dokunulmadı; sohbet penceresi yüksekliği `height: min(420px, 55vh)` ile viewport-duyarlı yapıldı. Build temiz, **73/73 test korundu** (saf CSS/Razor). Duman: giriş sayfası 200 (tema altyapısı HTML'de doğrulandı: FOUC script + site.css + site.js), anonim `/` → login 302; Razor view'ları build'de derlendiği için gruplu `_Layout` sözdizimi de doğrulandı.

### Faz 7 — Excel (.xlsx) ile Toplu Stajyer İçe Aktarma · **Tamamlandı [28.07.2026 09:10]**

- **Tamamlandı [28.07.2026 09:10]** — Admin/İK için toplu stajyer ekleme. Kararlar: format **Excel .xlsx** (ClosedXML 0.105.1, MIT — yerel kütüphane, ağ bağımlılığı değil); şablonda **amir sütunu yok** (stajyerler amirsiz eklenir, sonra "Eşleştir"); hata davranışı **geçerlileri ekle + hatalıları satır no/nedenle raporla**. (1) **Infrastructure** (`Services/TopluStajyer/`): `StajyerSablonu` (başlıklı .xlsx + "Talimatlar" sayfası, örnek veri satırı YOK), saf `TopluStajyerDogrulayici` (zorunlu alan, T.C. `^\d{11}$`, tarih sırası, dosya-içi + DB Kart No tekilliği), `TopluStajyerServisi` (`IceAktarAsync`): ClosedXML ile oku (tarih hücresi + `dd.MM.yyyy`/`yyyy-MM-dd` metin; sayısal T.C./kart hücreleri tam sayıya çevrilir), kullanıcı adı + kart no setleri **bir kez** çekilir, geçerliler **tek transaction**'da eklenir — üretilen her kullanıcı adı sete geri eklenerek batch-içi çakışma önlenir (bu yüzden `StajyerOlusturAsync` satır satır çağrılmaz, dedike döngü). Tek özet `DenetimKaydi`. **KVKK:** dosya bellekte işlenir, diske YAZILMAZ, T.C./içerik loglanmaz; T.C. yalnız geçici parola (`ForceChangePassword=true`). Satır sınırı 1000. (2) **Web:** `KullaniciYonetimController` (yalnız `AdminPolicy`) `SablonIndir` (GET → File) + `TopluEkle` (POST, `[ValidateAntiForgeryToken]`, `[RequestSizeLimit 5MB]`, uzantı/tip kontrolü, `dosya.OpenReadStream()`); `StajyerOlusturViewModel.TopluSonuc`; `StajyerOlustur.cshtml` üstüne "Toplu Ekleme" kartı (şablon indir + yükleme + sonuç tablosu; kullanıcı içeriği Razor ile encode). `Program.cs` DI. (3) **Testler:** `TopluStajyerDogrulayiciTests` (7 saf test) + `TopluStajyerServisiTests` (5 test: SQLite üstünde DI ile gerçek `UserManager` — amirsiz ekleme, mevcut kart no atlama, bozuk dosya, tarih/metin ayrıştırma, şablon başlıkları). Test projesine ClosedXML eklendi. Toplam **86/86 geçti**. Build temiz. Duman: uygulama sağlıklı açıldı, `SablonIndir` anonim erişimde login'e 302, giriş 200. Kimlik korumalı yükleme uçtan uca kullanıcı tarafında doğrulanacak (admin parolası kullanıcıda).
